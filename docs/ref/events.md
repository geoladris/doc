...
Oculta el indicador mostrado por [show-wait-mask](#show-wait-mask).
...
Enviado una vez el módulo `layers` ha lanzado los eventos [add-layer](#add-layer) y [add-group](#add-group) correspondientes a la configuración de capas existente en el servidor.
...
Como norma general, cualquier módulo que escuche los eventos [add-layer](#add-layer), [add-group](#add-group) o [layers-loaded](#layers-loaded) también deberá escuchar el evento `reset-layers`, y devolver el estado interno del módulo (y del DOM) al que tenía justo antes de empezar a cargar grupos y capas.
...
**Parámetros**: Función que devuelve el objeto jQuery que se mostará a modo de botón. Esta función toma como parámetro el mismo objeto que se lanza en el evento [add-layer](#add-layer).
...
Igual que [register-layer-action](#register-layer-action) pero para grupos.
...
* `features`: array con las features OpenLayers. Cada feature tiene:
    ...
    * Una propiedad `aliases` que es un array que contiene un objeto con propiedades _name_ y _alias_ para cada atributo de la feature. Por ejemplo:
